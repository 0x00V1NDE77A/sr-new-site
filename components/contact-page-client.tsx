...
const topicKeys = Object.keys(topicValues) as Array<keyof typeof topicValues>

export function ContactPageClient({ contactDetails }: ContactPageClientProps) {
  const t = useTranslations('Contact')

  const contactInfo = contactDetails || {
    phone: t('contactDetails.defaultPhone'),
    email: t('contactDetails.defaultEmail'),
    address: t('contactDetails.defaultAddress'),
  }

  const topicOptions = topicKeys.map((key) => ({
    value: topicValues[key],
    label: t(`form.topics.${key}` as const),
  }))
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.details && result.details.length > 0) {
          ...
          setErrors({ general: result.error || t('feedback.error') })
        }
      }
    } catch (error) {
      console.error('Error submitting form:', error)
      setErrors({ general: t('feedback.error') })
    } finally {
          ...
          >
            <div className="lg:w-1/2">
              <motion.div className="mb-8" variants={fadeInUp}>
                <h1
                  className="p-4 mb-6 text-5xl font-bold leading-tight text-black bg-white rounded-lg shadow-sm md:text-6xl"
              ...
              <motion.div className="space-y-6" variants={fadeInUp}>
                ...
                >
                  <h3 className="mb-3 text-lg font-semibold text-gray-900">{t('info.title')}</h3>
                  <motion.a
            ...
            >
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="pl-3 ml-auto">
                      <div className="-mx-1.5 -my-1.5">
                        ...
                        >
                          <span className="sr-only">{t('feedback.dismiss')}</span>
              ...
              >
                ...
                <div>
                  ...
                  >
                    {isSubmitting ? t('form.submitStates.submitting') : t('form.submitStates.idle')}
                  </button>
                </div>
              </motion.form>
            </motion.div>
