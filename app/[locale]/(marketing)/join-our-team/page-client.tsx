...
type PositionsCtaTranslation = {
  apply: string
  notListed: string
  resumeCta: string
}

type ClosingFormTranslation = {
  nameLabel: string
...
export default function JoinTeamPageClient() {
  ...
  const featureIcons = [Zap, Target, Award, TrendingUp]
  const benefitIcons = [Heart, Rocket, Coffee, Globe]

  const features = t.raw('features') as FeatureTranslation[]
  const benefits = t.raw('benefits') as BenefitTranslation[]
  const testimonials = t.raw('testimonials') as TestimonialTranslation[]
  const openPositions = t.raw('openPositions') as OpenPositionTranslation[]
  const positionsCta = t.raw('positionsCta') as PositionsCtaTranslation
  const closingForm = t.raw('closing.form') as ClosingFormTranslation

  const heroBadge = t('hero.badge')
  const heroTitlePrimary = t('hero.titlePrimary')
  const heroTitleSecondary = t('hero.titleSecondary')
  const heroDescription = t('hero.description')
  const heroPrimaryCta = t('hero.primaryCta')
  const heroSecondaryCta = t('hero.secondaryCta')

  const whyChooseTitle = t('whyChoose.title')
  const whyChooseDescription = t('whyChoose.description')

  const offerHeading = t('offerHeading')
  const testimonialsHeading = t('testimonialsHeading')
  const testimonialsDescription = t('testimonialsDescription')

  const positionsHeading = t('positionsHeading')
  const positionsDescription = t('positionsDescription')

  const closingHeading = t('closing.heading')
  const closingDescription = t('closing.description')
      ...
      <section className="py-20 text-black bg-white lg:py-32">
        <div className="px-4 mx-auto max-w-7xl sm:px-6">
          ...
          <motion.div
            ...
            transition={{ duration: 0.8 }}
          >
            <p className="mb-6 text-gray-600">{positionsCta.notListed}</p>
            <LiquidButton
              size="lg"
              className="px-8 py-3 text-white bg-black rounded-full hover:bg-black/90"
            >
              {positionsCta.resumeCta}
            </LiquidButton>
          </motion.div>
        </div>
      </section>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    >
                      ...
                      <input
                        ...
                        placeholder={closingForm.namePlaceholder}
                        required
                      />
                    </motion.div>
                    <motion.div
                    ...
                    >
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 text-white transition-all duration-300 border rounded-lg bg-white/10 border-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 placeholder-white/50"
                        placeholder={closingForm.emailPlaceholder}
                        required
                      />
                    </motion.div>
                  </div>
                  ...
                  >
                    ...
                    <textarea
                      ...
                      placeholder={closingForm.messagePlaceholder}
                      required
                    />
                  </motion.div>
                  <motion.div
