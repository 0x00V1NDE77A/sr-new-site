...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.mobileApps.metadata' })
  ...
  const seoData = await generateSEOMetadata(
    ...
    `/${normalizedLocale}/services/mobile-apps`
  )
...
export default async function MobileAppsService({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const normalizedLocale: AppLocale = isAppLocale(locale) ? locale : DEFAULT_LOCALE
  const translation = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.mobileApps' })
