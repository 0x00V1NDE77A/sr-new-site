...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.aiIntegration.metadata' })
  ...
  const seoData = await generateSEOMetadata(
    ...
    `/${normalizedLocale}/services/ai-integration`
  )
...
export default async function AIIntegrationService({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const normalizedLocale: AppLocale = isAppLocale(locale) ? locale : DEFAULT_LOCALE
  const translation = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.aiIntegration' })
