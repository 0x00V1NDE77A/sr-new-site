...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.webDevelopment.metadata' })
  ...
  const seoData = await generateSEOMetadata(
    ...
    `/${normalizedLocale}/services/web-development`
  )
...
export default async function WebDevelopmentService({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const normalizedLocale: AppLocale = isAppLocale(locale) ? locale : DEFAULT_LOCALE
  const translation = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.webDevelopment' })
