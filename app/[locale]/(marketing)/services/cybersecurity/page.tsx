...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.cybersecurity.metadata' })
  ...
  const seoData = await generateSEOMetadata(
    ...
    `/${normalizedLocale}/services/cybersecurity`
  )
...
export default async function CybersecurityService({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const normalizedLocale: AppLocale = isAppLocale(locale) ? locale : DEFAULT_LOCALE
  const translation = await getTranslations({ locale: normalizedLocale, namespace: 'ServiceDetail.cybersecurity' })
